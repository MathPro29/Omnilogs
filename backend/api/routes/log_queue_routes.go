...
func LogQueueRoutes(router *gin.Engine, db *gorm.DB, env *configs.Env, esClient *elasticsearch.Client, natsQueue *configs.NATSQueue) {
	...
	use := logqueuesusecase.NewUsecase(repo, natsQueue, logqueuesusecase.PayloadLimits{
		MaxPayloadSizeBytes: env.MaxLogPayloadSizeBytes,
		MaxJSONDepth:        env.MaxLogJSONDepth,
		MaxFieldCount:       env.MaxLogFieldCount,
		MaxStringLength:     env.MaxLogStringLength,
		MaxArrayLength:      env.MaxLogArrayLength,
	})
	...
	router.POST("/api/v1/logs/ingest", middleware.ProductAPIKeyAuthMiddleware(db), handler.QueueHandler)
	router.GET("/api/v1/ingest/batches/:batchId", middleware.ProductAPIKeyAuthMiddleware(db), handler.GetIngestBatchStatusHandler)
