...
import (
	...
	routinghandler "omnilogs-api/internal/log_routing/handler"
	...
	retentionPolicyHandler "omnilogs-api/internal/retention_policy/handler"
	retentionPolicyRepo "omnilogs-api/internal/retention_policy/repository"
	retentionPolicyUsecase "omnilogs-api/internal/retention_policy/usecase"
...
func ProductRoutes(router *gin.Engine, db *gorm.DB, env *configs.Env) {
	...
	products.POST("/setup", productHandler.CreateProduct)
	products.GET("", productHandler.ListProducts)
	products.GET("/:productId", productHandler.GetProduct)
	products.GET("/:productId/setup-status", productHandler.GetSetupStatus)
	products.PATCH("/:productId/setup/product", productHandler.UpdateProduct)
	products.POST("/:productId/setup/validate", productHandler.ValidateSetupStructure)
	products.POST("/:productId/setup/complete", productHandler.CompleteSetup)
	products.PATCH("/:productId", productHandler.UpdateProduct)
	products.DELETE("/:productId", productHandler.DeleteProduct)
	products.POST("/:productId/restore", productHandler.RestoreProduct)
	products.DELETE("/product/bulk-delete", productHandler.BulkDeleteProducts)

	authorizer := globalauthusecase.NewUsecase(globalauthrepo.NewRepository(db))
	archiveHandler := logArchiveHandler.NewHandlerWithService(logArchiveService.New(db, esClient, authorizer))
	routingHandler := routinghandler.New(db)
	products.GET("/:productId/log-archives", archiveHandler.List)
	products.POST("/:productId/log-archives", archiveHandler.Create)
	products.GET("/:productId/log-archives/download", archiveHandler.Download)
	products.GET("/:productId/log-archive-downloads", archiveHandler.ListReadyDownloads)
	products.GET("/:productId/log-archive-downloads/:downloadId", archiveHandler.DownloadReady)
	products.POST("/:productId/log-archives/import", archiveHandler.Import)
	products.GET("/:productId/log-archives/:archiveId", archiveHandler.Get)
	products.POST("/:productId/log-archives/:archiveId/verify", archiveHandler.Verify)
	products.POST("/:productId/log-archives/:archiveId/restore", archiveHandler.Restore)
	products.GET("/:productId/archive-jobs", archiveHandler.ListJobs)
	products.GET("/:productId/archive-jobs/:jobId", archiveHandler.GetJob)
	...
	products.GET("/:productId/log-routes", routingHandler.List)
	products.POST("/:productId/log-routes", routingHandler.Create)
	products.DELETE("/:productId/log-routes/:routeId", routingHandler.Delete)
	products.GET("/:productId/log-routing-rules", routingHandler.ListRules)
	products.GET("/:productId/routing-discovery", routingHandler.ListDiscovery)
	products.POST("/:productId/log-routing-rules", routingHandler.CreateRule)
	products.PUT("/:productId/log-routing-rules/:ruleId", routingHandler.UpdateRule)
	products.DELETE("/:productId/log-routing-rules/:ruleId", routingHandler.DeleteRule)
	...
	retentionHandler := retentionPolicyHandler.NewHandler(
		retentionPolicyUsecase.NewUsecase(retentionPolicyRepo.NewRepository(db, esClient), authorizer),
	)
	retentionGroup := products.Group("/:productId/retention-policies")
	retentionGroup.Use(middleware.UserAuthMiddleware(env.JWTSecret))
	retentionGroup.POST("", retentionHandler.Create)
	retentionGroup.GET("", retentionHandler.List)
	retentionGroup.GET("/stats", retentionHandler.GetStats)
	retentionGroup.POST("/simulate", retentionHandler.Simulate)
	retentionGroup.GET("/historical", retentionHandler.Historical)
	retentionGroup.POST("/preview", retentionHandler.Preview)
	retentionGroup.GET("/:policyId", retentionHandler.GetByID)
	retentionGroup.PUT("/:policyId", retentionHandler.Update)
	retentionGroup.DELETE("/:policyId", retentionHandler.Delete)
	retentionGroup.PATCH("/:policyId/toggle", retentionHandler.ToggleActive)
	retentionGroup.POST("/:policyId/run-now", retentionHandler.TriggerNow)
}
