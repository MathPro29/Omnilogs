...
type IngestLogItemRequest struct {
	...
	// CustomFields lets a source send its key:value classification separately
	// from the event body. The queue normalizes it into the indexed document.
...
type IngestLogBatchRequest struct {
	...
	DetectedProductCode *string                `json:"detected_product_code,omitempty"`
	HeaderProjectID     string                 `json:"-"`
	HeaderProjectCode   string                 `json:"-"`
	HeaderEnvironment   string                 `json:"-"`
