...
type Repository interface {
	...
	ListActions(ctx context.Context, filter dto.AuditLogFilterRequest) ([]string, error)
}
...
func (r *repository) ListActions(ctx context.Context, filter dto.AuditLogFilterRequest) ([]string, error) {
	query := r.db.WithContext(ctx).Model(&models.SystemAuditLog{})
	if filter.AllowedProductIDs != nil {
		if len(filter.AllowedProductIDs) == 0 {
			return []string{}, nil
	...
	var actions []string
	if err := query.Where("action <> ''").Distinct().Order("action ASC").Pluck("action", &actions).Error; err != nil {
		return nil, err
	}
	return actions, nil
}
