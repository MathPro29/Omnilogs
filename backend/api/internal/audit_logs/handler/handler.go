...
func (h *Handler) ListActions(c *gin.Context) {
	...
	values, err := h.usecase.ListActions(c.Request.Context(), req)
	if err != nil {
		responses.Error(c, "INTERNAL_ERROR", "failed to query audit activities", err)
		return
	}
	responses.Success(c, http.StatusOK, "AUDIT_ACTIVITIES_RETRIEVED", values)
}
