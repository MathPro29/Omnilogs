...
func (h *Handler) Register(c *gin.Context) {
	...
	c.Set("audit_action", "USER.REGISTERED")
	c.Set("audit_resource_type", "USER")
	c.Set("audit_resource_id", fmt.Sprintf("%d", user.UserID))
...
func (h *Handler) Login(c *gin.Context) {
	...
	c.Set("audit_action", "USER.LOGIN")
	c.Set("audit_resource_type", "USER")
	c.Set("audit_resource_id", fmt.Sprintf("%d", tokens.UserID))
...
func (h *Handler) RefreshToken(c *gin.Context) {
	...
	c.Set(middleware.ContextUserID, tokens.UserID)
	c.Set("audit_action", "USER.SESSION_REFRESHED")
	c.Set("audit_resource_type", "USER")
	c.Set("audit_resource_id", fmt.Sprintf("%d", tokens.UserID))
...
func (h *Handler) Logout(c *gin.Context) {
	...
	userID, err := h.usecase.Logout(req.RefreshToken)
	if err != nil {
		if errors.Is(err, responses.ErrorUserCode["INVALID_REFRESH_TOKEN"]) {
			responses.Unauthorized(c, "invalid refresh token")
			return
	...
	c.Set("audit_action", "USER.LOGOUT")
	c.Set("audit_resource_type", "USER")
	c.Set("audit_resource_id", fmt.Sprintf("%d", userID))
