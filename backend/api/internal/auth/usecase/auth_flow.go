...
func (u *usecase) Logout(refreshToken string) (uint, error) {
	hash := utils.SHA256Hex(refreshToken)
	session, err := u.repo.FindActiveSessionByRefreshTokenHash(hash)
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return 0, responses.ErrorUserCode["INVALID_REFRESH_TOKEN"]
	}
	if err != nil {
		return 0, err
	}
	...
	if err := u.repo.RevokeSessionByRefreshTokenHash(hash, time.Now()); err != nil {
		return 0, err
	}
