...
func (h *Handler) CreateMembership(c *gin.Context) {
	...
	c.Set("audit_action", "PRODUCT_MEMBER.ADDED")
	c.Set("audit_resource_type", "PRODUCT_MEMBERSHIP")
	c.Set("audit_resource_id", fmt.Sprintf("%d", value.UserID))
	c.Set("audit_reason", fmt.Sprintf("Added user %d to product %d", value.UserID, productID))
	c.Set("audit_payload", gin.H{"activity": "MEMBER_ADDED", "membership": value})
	utils.Success(c, http.StatusCreated, value)
}
...
func (h *Handler) CreateMemberships(c *gin.Context) {
	...
	c.Set("audit_action", "PRODUCT_MEMBER.BULK_ADDED")
	c.Set("audit_resource_type", "PRODUCT_MEMBERSHIP")
	c.Set("audit_reason", fmt.Sprintf("Added %d users to product %d", len(values), productID))
	c.Set("audit_payload", gin.H{"activity": "MEMBERS_BULK_ADDED", "memberships": values})
...
func (h *Handler) UpdateMembership(c *gin.Context) {
	...
	c.Set("audit_action", "PRODUCT_MEMBER.UPDATED")
	c.Set("audit_resource_type", "PRODUCT_MEMBERSHIP")
	c.Set("audit_resource_id", fmt.Sprintf("%d", value.UserID))
	c.Set("audit_reason", fmt.Sprintf("Updated membership for user %d in product %d", value.UserID, productID))
	c.Set("audit_payload", gin.H{"activity": "MEMBER_UPDATED", "membership": value})
	utils.Success(c, http.StatusOK, value)
}
...
func (h *Handler) DeleteMembership(c *gin.Context) {
	...
	c.Set("audit_action", "PRODUCT_MEMBER.REMOVED")
	c.Set("audit_resource_type", "PRODUCT_MEMBERSHIP")
	c.Set("audit_resource_id", fmt.Sprintf("%d", id))
	c.Set("audit_reason", fmt.Sprintf("Removed membership %d from product %d", id, productID))
	c.Set("audit_payload", gin.H{"activity": "MEMBER_REMOVED", "membership_id": id})
