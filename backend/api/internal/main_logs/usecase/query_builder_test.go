...
func TestBuildSearchQueryNormalizesCustomFieldPath(t *testing.T) {
	...
	text := string(encoded)
	if !strings.Contains(text, `data.custom_fields.customer_tier.keyword`) {
...
func TestBuildSearchQueryAddsOperationalFilters(t *testing.T) {
	dateFrom := time.Date(2026, 7, 22, 12, 0, 0, 0, time.UTC)
	statusCode := int64(500)
	sourceID := int64(9)
	query := buildSearchQuery(SearchInput{
		ProductID: 1, DateFrom: &dateFrom, StatusCode: &statusCode, SourceID: &sourceID,
		Page: 1, PerPage: 20,
	})
	encoded, err := json.Marshal(query)
	if err != nil {
	...
	text := string(encoded)
	for _, expected := range []string{"\"range\"", "\"payload.status_code\"", "\"source_id\"", "\"@timestamp\""} {
		if !strings.Contains(text, expected) {
			t.Fatalf("expected filter %s in query: %s", expected, text)
		}
	}
}
func TestBuildSearchQueryMatchesLogLevelAcrossIndexMappings(t *testing.T) {
	level := "INFO"
	query := buildSearchQuery(SearchInput{
		ProductID: 1, Level: &level, Page: 1, PerPage: 20,
	})
	encoded, err := json.Marshal(query)
	if err != nil {
		t.Fatalf("marshal query: %v", err)
	...
	for _, expected := range []string{
		`"payload.log_level.keyword"`,
	} {
		if !strings.Contains(text, expected) {
			t.Fatalf("expected level fallback %s in query: %s", expected, text)
		}
	}
	if !strings.Contains(text, `"INFO"`) {
		t.Fatalf("expected case-compatible level values in query: %s", text)
	}
}
