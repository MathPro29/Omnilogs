...
type Repository interface {
	...
	GetRestoredArchiveIndexPattern(ctx context.Context, productID int64, environmentID *int64, archiveID string) (string, error)
}
...
func (r *repository) HasMainLogAccess(ctx context.Context, userID uint, productID int64) (bool, error) {
	...
	if err := r.db.WithContext(ctx).Model(&models.ProductMembership{}).
		Where("user_id = ? AND product_id = ? AND is_active = TRUE AND (expires_at IS NULL OR expires_at > NOW())", userID, productID).
		Count(&activeMemberships).Error; err != nil {
		return false, err
	}
...
func (r *repository) GetRestoredArchiveIndexPattern(ctx context.Context, productID int64, environmentID *int64, archiveID string) (string, error) {
	var archive models.LogArchive
	query := r.db.WithContext(ctx).
		Where("archive_id = ? AND product_id = ? AND status IN ? AND restore_status = ?", archiveID, productID, []string{"VERIFIED", "SNAPSHOT_ONLY"}, "RESTORED")
	...
	if archive.RestoreIndexPattern == nil || *archive.RestoreIndexPattern == "" {
		return "", gorm.ErrRecordNotFound
	}
	return *archive.RestoreIndexPattern, nil
}
