...
func TestMapFallsBackToCanonicalDocumentPath(t *testing.T) {
	mapped := Map("log-1", map[string]any{
		"product_id": int64(7),
		"method":     "POST",
		"path":       "/api/bill_others/bot_settings/add",
		"payload":    map[string]any{"message": "created"},
	})
	if mapped.Method == nil || *mapped.Method != "POST" || mapped.Path == nil || *mapped.Path != "/api/bill_others/bot_settings/add" {
		t.Fatalf("unexpected HTTP mapping: %+v", mapped)
	}
}
