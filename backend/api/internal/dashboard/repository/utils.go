...
func (r *repository) resolveSearchIndices(ctx context.Context, productID, environmentID int) []string {
	indices := []string{fmt.Sprintf("omnilogs-product-%d-env-%d-search-v3-*", productID, environmentID)}
	...
	if err := r.db.WithContext(ctx).
		...
		for _, policy := range policies {
			if strings.TrimSpace(policy.IndexPrefix) != "" {
				...
				pattern := fmt.Sprintf("%s-env-%d-search-v3-*", prefix, environmentID)
