...
func (h *Handler) FavoriteJSONField(c *gin.Context) {
	...
	pathCandidatesSet := make(map[string]struct{})
	for _, candidate := range []string{
		...
		if candidate != "" {
			pathCandidatesSet[candidate] = struct{}{}
		}
	}
	pathCandidates := make([]string, 0, len(pathCandidatesSet))
	for candidate := range pathCandidatesSet {
	...
	if err := keyQuery.Where("field_key = ? OR field_key LIKE ?", baseFieldKey, baseFieldKey+"_%").Pluck("field_key", &usedFieldKeys).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func normalizeFavoriteFieldPath(path string) string {
	...
	if path == "payload" || path == "data" || path == "fields" || path == "" || path == "$" {
		return "$"
	}
	return path
}
