...
func ProductAPIKeyAuthMiddleware(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !apiKeyAllowsPermission(apiKey.Permissions, "LOG_INGEST_CREATE") {
			responses.Forbidden(c, "API key does not allow log ingestion")
			c.Abort()
			return
		}
		...
		applyAPIKeyIdentity(c, apiKey)
		c.Next()
	}
}

func applyAPIKeyIdentity(c *gin.Context, apiKey models.ProductAPIKey) {
	c.Set("service_product_id", apiKey.ProductID)
	c.Set("service_api_key_id", apiKey.KeyID)
	if apiKey.EnvironmentID != nil {
		c.Set("service_environment_id", *apiKey.EnvironmentID)
	}
	if apiKey.SourceID != nil {
		c.Set("service_source_id", *apiKey.SourceID)
	}
}

func apiKeyAllowsPermission(raw json.RawMessage, required string) bool {
