...
export function DashboardPage() {
  ...
  const handleToggleAutoSending = async () => {
    ...
    try {
      ...
      startAutoSending({
        ...
        simulationMode: allValues.simulationMode || 'SUCCESS',
      });
  ...
  // Failed Batches Query
  const { data: failedBatchesResponse, isLoading: isLoadingFailedBatches, refetch: refetchFailedBatches } = useQuery({
    queryKey: ['failed-batches'],
    queryFn: () => productAdminService.getFailedBatches(),
    refetchInterval: 10000, // refresh every 10s
  });
  const failedBatches = failedBatchesResponse?.data || [];
          ...
          <Col xs={24} lg={8}>
            <Space direction="vertical" style={{ width: '100%' }} size="middle">
              ...
              >
                ...
                <Form.Item
                  label="โหมดการทำงานจำลอง (Ingestion Simulation Mode)"
                  name="simulationMode"
                  initialValue="SUCCESS"
                >
                  <Select
                    options={[
                      { value: 'SUCCESS', label: 'ทำงานปกติ (ส่งผ่านปกติ 100%)' },
                      { value: 'MIXED_FAILURES', label: 'จำลองสุ่มข้อผิดพลาด (ปกติ 70% / ล้มเหลว 30%)' },
                      { value: 'ALL_FAILURES', label: 'จำลองล้มเหลวทั้งหมด (ล้มเหลว 100%)' },
                    ]}
                  />
                </Form.Item>
                ...
                <Space direction="vertical" style={{ width: '100%' }}>
                  <Button 
                    ...
                    onClick={() => {
                      ...
                      if(!vals.productId || !vals.environmentId) { message.warning('กรุณาเลือก Product และ Environment ก่อน'); return; }
                      generateLogMutation.mutate({
                        productId: vals.productId,
                        environmentId: vals.environmentId,
                        projectId: vals.projectId || undefined,
                        categoryId: vals.categoryId || undefined,
                        logLevel: vals.logLevel || 'INFO',
                        eventType: vals.eventType || 'manual-test',
                        message: 'Case 1: Valid Log Ingestion Success',
                      });
                    }}
                  >
                    Case 1: Valid Log Ingestion (สำเร็จ)
                  </Button>
                  <Button 
                    block 
                    onClick={() => {
                  ...
                  >
                    Case 2: Invalid Environment (ผิด Product หรือไม่มีจริง)
                  </Button>
                  <Button 
                    block 
                    onClick={() => {
                  ...
                  >
                    Case 4: Project Mismatch / Inactive Project (Project ปิดใช้งานหรือไม่มีอยู่จริง)
                  </Button>
                  <Button 
                    block 
                    onClick={() => {
                  ...
                  >
                    Case 5: Category Mismatch / Inactive Category (Category ปิดใช้งานหรือไม่มีอยู่จริง)
                  </Button>
                  <Button 
                    block 
                    onClick={() => {
                      const vals = generatorForm.getFieldsValue();
                      if(!vals.environmentId) { message.warning('กรุณาเลือก Environment ก่อน'); return; }
                      apiClient.post('/queues', {
                        environment_id: vals.environmentId,
                        queue_key: 'test-missing-product-id',
                        source_type: 'WEB',
                        ...
                        logs: [
                          {
                            ...
                            input_payload: {
                              log_level: 'ERROR',
                              message: 'Test: Missing Product ID Ingestion Error',
                              timestamp: new Date().toISOString(),
                            },
                          },
                        ],
                      })
                      .then(() => message.success('ส่งสำเร็จ (ซึ่งไม่ควรผ่าน)'))
                      .catch((err: any) => message.error(`พบข้อผิดพลาดตามคาด: ${err?.response?.data?.message || err?.message || err}`));
                    }}
                  >
                    Case 6: Missing Product ID (มี Env ID แต่ไม่มี Product ID)
                  </Button>
                  <Button 
                    block 
                    onClick={() => {
                  ...
                  >
                    Case 7: Empty Logs Array (ไม่มีรายการ Logs ส่งมา)
                  </Button>
                </Space>
              </div>
            </Card>
              ...
              title={
                <Space style={{ width: '100%', justifyContent: 'space-between' }}>
                  <Space>
                    ...
                    <span style={{ fontWeight: 600 }}>ระบบจัดการ Failed Logs & Retry</span>
                  </Space>
                  <Button 
                    type="text" 
                    size="small"
                    icon={<ReloadOutlined spin={isLoadingFailedBatches} />} 
                    onClick={() => refetchFailedBatches()} 
                  />
                </Space>
              }
              bordered={false}
            ...
            >
              <Paragraph type="secondary" style={{ fontSize: '13px' }}>
                ติดตามประวัติและสั่งประมวลผลข้อความ Log ในตาราง <Text code>log_failures</Text> ที่ประมวลผลล้มเหลวแบบถาวรกลับเข้าสู่ระบบใหม่
              </Paragraph>
              ...
              {failedBatches.length > 0 ? (
                <div style={{ marginBottom: '16px' }}>
                  <Alert
                    type="error"
                    showIcon
                    message={`พบ ${failedBatches.length} บล็อกที่ล้มเหลวถาวร (FAILED)`}
                    description="กรุณาตรวจสอบโครงสร้างข้อมูลหรือสถานะระบบ แล้วสั่ง Retry อีกครั้งด้านล่าง"
                    style={{ marginBottom: '12px' }}
                  />
                  <div style={{ maxHeight: '250px', overflowY: 'auto', border: '1px solid #f0f0f0', borderRadius: '6px', padding: '8px', backgroundColor: '#fafafa', marginBottom: '16px' }}>
                    {failedBatches.map((b: any, index: number) => (
                      <div key={b.batch_id} style={{ borderBottom: index < failedBatches.length - 1 ? '1px solid #e8e8e8' : 'none', padding: '6px 0', fontSize: '12px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <Text strong style={{ fontFamily: 'monospace' }}>Batch: {b.batch_id.slice(0, 8)}...</Text>
                          <Tag color="red">FAILED</Tag>
                        </div>
                        <div style={{ color: '#8c8c8c' }}>
                          ลองแล้ว: {b.processing_attempts} ครั้ง | จำนวนคิว: {b.total_logs}
                        </div>
                        {b.error_message && (
                          <div style={{ color: '#cf1322', marginTop: '4px', fontSize: '11px', fontFamily: 'monospace', wordBreak: 'break-all' }}>
                ...
                <Alert
                  type="success"
                  showIcon
                  message="ไม่มีคิวข้อความที่ล้มเหลวตกค้าง"
                  style={{ marginBottom: '16px' }}
                />
              )}
            </Card>
