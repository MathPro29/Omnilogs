...
function getQueueStatusMeta(items: LogQueueItem[]) {
  ...
  if (items.some((item) => item.status === 'RETRY_PENDING' || item.status === 'PROCESSING')) {
    return { label: 'Retrying', color: 'warning' as const, description: 'Queue is retrying or processing items.' };
  }
...
export function ProductCatalogPage() {
  ...
  const batchItemsQuery = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      const hasActiveWork = items.some((item) => item.status === 'PENDING' || item.status === 'PROCESSING' || item.status === 'RETRY_PENDING');
                              ...
                              <List.Item>
                                <div className="w-full space-y-1">
                                  <div className="flex items-center justify-between gap-3">
                                    ...
                                    <Tag color={item.status === 'FAILED' ? 'error' : item.status === 'RETRY_PENDING' ? 'warning' : item.status === 'PROCESSED' ? 'success' : 'processing'}>
                                      {item.status === 'RETRY_PENDING' ? 'Retrying' : item.status}
                                    </Tag>
                                  </div>
                                  <Text type="secondary">
                                    {item.status === 'RETRY_PENDING' && item.nextRetryAt
                                      ? `Retry ${item.retryCount}/${item.maxRetryCount} at ${item.nextRetryAt}`
                                      : item.processedAt
